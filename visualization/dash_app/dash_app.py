...
COVID_IMG = "https://unsplash.com/ko/%EC%82%AC%EC%A7%84/k0KRNtqcjfw"
...
new_confirmed = df_global["NewConfirmed"][0]
deaths = df_global["TotalDeaths"][0]
new_deaths = df_global["NewDeaths"][0]
recovered = df_global["TotalRecovered"][0]
new_recovered = df_global["NewRecovered"][0]
...
df_world_f = pd.merge(
    df_countries[
        ["Country", "TotalConfirmed", "TotalDeaths", "TotalRecovered", "CountryCode"]
    ],
    code_mapping,
    left_on="CountryCode",
    right_on="alpha-2",
        ...
        dbc.CardBody(
            [
                dcc.Markdown(
                    ...
                    children=[
                        "{0} <br><sub>+{1}</sub></br>".format(total_cases, new_cases)
                    ],
                )
            ]
        ),
...
body_app = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    dbc.Card(
                        data_for_cases(
                            "Confirmed", f"{confirmed:,}", f"{new_confirmed:,}"
                        ),
                        color="primary",
                ...
                dbc.Col(
                    dbc.Card(
                        data_for_cases(
                            "Recovered", f"{recovered:,}", f"{new_recovered:,}"
                        ),
                        color="success",
                        style={"text-align": "center"},
                        inverse=True,
                ...
                dbc.Col(
                    dbc.Card(
                        data_for_cases("Deaths", f"{deaths:,}", f"{new_deaths:,}"),
                        color="danger",
                        style={"text-align": "center"},
                        inverse=True,
                    ),
        ...
        dbc.Row(
            [
                html.Div(
                    html.H4("Covid-19 Worldwide Impact"),
                    style={
                        "textAlign": "center",
                        "fontWeight": "bold",
                        "family": "georgia",
        ...
        dbc.Row(
            [
                dbc.Col(
                    dcc.Graph(id="world-graph", figure=world_map(df_world_f)),
                    style={"height": "450px"},
                    xs=12,
                    sm=12,
                    md=6,
                            ...
                            children=[
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        {"label": i, "value": i}
                                        for i in np.append(
                                            ["All"], df_countries["Country"].unique()
                                        )
                                    ],
                                    value="All",
...
@app.callback(
    Output(component_id="world-table-output", component_property="children"),
